...
5. HTTP: 3xx with a second local listener gets no redirected request. Source
   review verifies the client is configured with no proxy and with system CAs
   disabled; unrelated-root and redirect fixtures exercise the resulting
   selected trust and endpoint boundaries.
