...
class AdminDialog final : public termforge::Dialog {
 public:
  ...
  [[nodiscard]] auto enable_displayed_container_logs(std::string_view container)
      -> std::expected<void, surfaces::ManualOpsFailure>;
 ...
 private:
  ...
  auto select_pod_log_source(int row) -> void;
  ...
  [[nodiscard]] auto displayed_log_source() const
      -> std::optional<surfaces::AdminDisplayedLogSource>;
  ...
  termforge::ListWidget m_containers;
  termforge::TextBox m_evidence;
  termforge::Button m_use{"[ Use target ]"};
  termforge::Button m_read{"[ Read ]"};
  termforge::Button m_enable_logs{"[ Allow logs ]"};
  termforge::Button m_disable_logs{"[ Revoke logs ]"};
  termforge::Button m_read_logs{"[ Read logs ]"};
  ...
  std::vector<std::string> m_pod_container_labels;
  std::vector<surfaces::AdminDisplayedLogSource> m_pod_log_sources;
  ...
  std::array<std::optional<surfaces::AdminDisplayedLogSource>, 2> m_log_sources;
  std::optional<surfaces::AdminDisplayedLogSource> m_active_log_source;
  ...
  std::string m_compact_log{"log unavailable"};
  std::string m_body;
};
} // namespace aiforge::adapters
