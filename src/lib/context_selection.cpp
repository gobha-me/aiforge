...
[[nodiscard]] auto failure(
    const ContextSelectionErrorCode code, std::string message,
  ...
  return std::unexpected(ContextSelectionError{
      code, std::move(message), std::move(entry_id), std::move(evidence_id)});
}
...
[[nodiscard]] auto valid_phase(const TaskPhase phase) noexcept -> bool {
  switch (phase) {
    ...
    case TaskPhase::review: return true;
    case TaskPhase::unknown: return false;
  }
  return false;
}

[[nodiscard]] auto known_budget_class(
    const ContextBudgetClass budget_class) noexcept -> bool {
  switch (budget_class) {
...
[[nodiscard]] auto known_representation(
    const ContextRepresentation representation) noexcept -> bool {
  switch (representation) {
    ...
    case ContextRepresentation::unknown: return true;
  }
  return false;
}

[[nodiscard]] auto known_freshness(const EvidenceFreshness freshness) noexcept
    -> bool {
  switch (freshness) {
...
[[nodiscard]] auto budget_limit(const ContextClassBudgets& budgets,
  ...
  switch (budget_class) {
    case ContextBudgetClass::conversation: return budgets.conversation_tokens;
    case ContextBudgetClass::summary: return budgets.summary_tokens;
...
[[nodiscard]] auto usage_value(ContextClassUsage& usage,
                               const ContextBudgetClass budget_class)
    -> std::uint64_t& {
  switch (budget_class) {
    case ContextBudgetClass::conversation: return usage.conversation_tokens;
    case ContextBudgetClass::summary: return usage.summary_tokens;
...
[[nodiscard]] auto contains_unknown(const std::vector<ContentBlock>& content)
    -> bool {
...
[[nodiscard]] auto contains_artifact(const std::vector<ContentBlock>& content)
    -> bool {
  return std::ranges::any_of(content, [](const ContentBlock& block) {
...
[[nodiscard]] auto reference_location(
    const RepositoryEvidenceReference& reference) -> std::string {
  return std::visit(
...
[[nodiscard]] auto reference_digest(
    const ContextParcelItem& item,
    const RepositorySnapshotIdentity& target_snapshot) -> std::string {
  if (const auto* exact = std::get_if<ExactSourceEvidence>(&item.reference)) {
    return exact->source.content_digest.algorithm + ':' +
           exact->source.content_digest.value;
  }
  const auto& snapshot =
      item.provenance.source_snapshot.value_or(target_snapshot);
  return snapshot.fingerprint.algorithm + ':' + snapshot.fingerprint.value;
}

[[nodiscard]] auto evidence_kind_rank(
    const std::optional<RepositoryEvidenceReference>& reference,
...
[[nodiscard]] auto class_rank(const ContextBudgetClass budget_class,
                              const TaskPhase phase) -> std::uint32_t {
...
[[nodiscard]] auto representation_rank(
    const ContextRepresentation representation, const TaskPhase phase)
    -> std::uint32_t {
  ...
  switch (phase) {
    ...
    case TaskPhase::unknown: return 7;
  }
  return 7;
}

[[nodiscard]] auto eligible_freshness(const EvidenceFreshness freshness,
                                      const TaskPhase phase) -> bool {
...
[[nodiscard]] auto same_exact_location(const CandidateState& left,
                                       const CandidateState& right) -> bool {
  ...
  const auto* right_source =
      std::get_if<ExactSourceEvidence>(&*right.reference);
  return left_source && right_source &&
         left_source->source.snapshot == right_source->source.snapshot &&
         left_source->source.relative_path ==
             right_source->source.relative_path &&
         left_source->source.range == right_source->source.range;
}

[[nodiscard]] auto valid_reference_representation(
    const CandidateState& candidate) -> bool {
        ...
        if constexpr (std::same_as<Reference, ExactSourceEvidence>) {
          ...
          return candidate.representation == ContextRepresentation::derived ||
                 ...
                 candidate.representation ==
                     ContextRepresentation::artifact_reference;
        } else if constexpr (std::same_as<Reference,
...
[[nodiscard]] auto unsupported_candidate(const CandidateState& candidate)
    -> bool {
  return candidate.budget_class == ContextBudgetClass::unknown ||
         ...
         (candidate.reference &&
          std::holds_alternative<UnknownRepositoryEvidence>(
              *candidate.reference)) ||
         (candidate.representation ==
              ContextRepresentation::artifact_reference &&
...
[[nodiscard]] auto decision_for_freshness(const EvidenceFreshness freshness)
  ...
  return freshness == EvidenceFreshness::unavailable
             ? ContextSelectionDecision::omitted_unavailable
...
auto ContextBuilder::select_and_build(ContextSelectionRequest request) const
  ...
  try {
    ...
    if (request.candidates.size() >
            request.selection_limits.maximum_candidates ||
    ...
    auto mandatory =
        build(ContextBuildInput{request.capacity, request.instructions, {}});
    if (!mandatory) {
    ...
    for (auto& candidate : request.candidates) {
      candidates.push_back(CandidateState{
          std::move(candidate.content), candidate.budget_class,
          candidate.representation, candidate.freshness, candidate.required,
          candidate.relevance_rank, std::move(candidate.alternative_group),
    ...
    for (auto& selection : request.parcels) {
      ...
      const auto validated = repository::validate_context_parcel(
          selection.parcel, request.parcel_limits);
      if (!validated) {
      ...
      if (selection.parcel.phase != request.phase) {
        return failure(
            ContextSelectionErrorCode::invalid_parcel,
            "repository parcel phase does not match selection phase");
      }
      if (target_snapshot &&
          !same_source_state(*target_snapshot,
                             selection.parcel.target_snapshot)) {
      ...
      for (auto& binding : selection.items) {
        if (!bindings.emplace(binding.evidence_id, &binding).second) {
          return failure(ContextSelectionErrorCode::duplicate_candidate,
                         "repository evidence bindings must be unique",
                         binding.entry_id, binding.evidence_id);
        }
      }
      ...
      for (auto& item : selection.parcel.items) {
        ...
        if (found == bindings.end()) {
          return failure(ContextSelectionErrorCode::invalid_parcel,
                         "parcel item is missing its selection binding",
                         std::nullopt, item.evidence_id);
        }
        ...
        if (binding.order == 0) {
          return failure(ContextSelectionErrorCode::invalid_candidate,
                         "repository evidence order must be positive",
                         binding.entry_id, binding.evidence_id);
        }
        ContextProvenance provenance{
            binding.source_id, reference_location(item.reference),
            reference_digest(item, selection.parcel.target_snapshot)};
        candidates.push_back(
            CandidateState{{binding.entry_id,
                            ContextContentKind::evidence,
                            {binding.message_id, Role::evidence,
                             std::move(item.content), std::nullopt},
                            std::move(provenance),
                            binding.order,
    ...
    for (std::size_t index{}; index < candidates.size(); ++index) {
      ...
      if (!known_budget_class(candidate.budget_class) ||
          ...
          !known_freshness(candidate.freshness) ||
          !valid_reference_representation(candidate)) {
        return failure(ContextSelectionErrorCode::invalid_candidate,
      ...
      if (!entry_ids.insert(candidate.content.entry_id).second ||
          ...
          (candidate.evidence_id &&
           !evidence_ids.insert(*candidate.evidence_id).second)) {
        return failure(ContextSelectionErrorCode::duplicate_candidate,
                       "context selection identities must be unique",
                       candidate.content.entry_id, candidate.evidence_id);
      }
      if (candidate.content.order == 0 ||
          ...
          !valid_group(
              candidate.alternative_group,
              request.selection_limits.maximum_alternative_group_bytes)) {
        return failure(
            ContextSelectionErrorCode::invalid_candidate,
      ...
      if (candidate.required &&
          candidate.freshness == EvidenceFreshness::current &&
          candidate.reference &&
      ...
      for (std::size_t previous{}; previous < index; ++previous) {
        ...
        const bool same =
            ...
            candidate.content.message.content ==
                candidates[previous].content.message.content &&
    ...
    if (request.phase == TaskPhase::editing && !has_required_current_exact) {
      return failure(
          ContextSelectionErrorCode::required_candidate_unavailable,
    ...
    std::ranges::sort(selectable, [&](const std::size_t left_index,
      ...
      const auto right_kind =
          evidence_kind_rank(right.reference, request.phase);
      ...
      const auto left_representation =
          representation_rank(left.representation, request.phase);
      const auto right_representation =
          representation_rank(right.representation, request.phase);
    ...
    const auto maximum_input = request.capacity.context_window_tokens -
                               request.capacity.reserved_output_tokens;
    ...
    for (const auto index : selectable) {
      ...
      if (candidate.alternative_group &&
          admitted_groups.contains(*candidate.alternative_group)) {
        decisions.push_back(make_record(
            candidate, ContextSelectionDecision::omitted_alternative));
        continue;
      }
      ...
      if (!class_fits || !total_fits) {
        if (candidate.required) {
          return failure(ContextSelectionErrorCode::mandatory_capacity_exceeded,
                         !class_fits
                             ? "required context exceeds its class token budget"
                             : "required context exceeds model input capacity",
                         candidate.content.entry_id, candidate.evidence_id);
        }
        decisions.push_back(make_record(
            candidate, class_fits
                           ? ContextSelectionDecision::omitted_budget
      ...
      decisions.push_back(
          make_record(candidate, ContextSelectionDecision::admitted));
      selected.push_back(std::move(candidate.content));
    }

    auto built = build(ContextBuildInput{request.capacity,
                                         std::move(request.instructions),
                                         std::move(selected)});
    if (!built) {
      return failure(context_error_code(built.error().code),
                     "selected context failed final validation",
                     built.error().entry_id);
    }
    std::ranges::sort(decisions, [](const auto& left, const auto& right) {
      return left.entry_id < right.entry_id;
    });
    return ContextSelectionResult{std::move(*built), std::move(decisions),
                                  usage};
  } catch (...) {
