...
[[nodiscard]] auto parse_arguments(const int argc, char* argv[])
  ...
  return saw_source && saw_output && !result.source_sha.empty() &&
                 !result.output.empty()
...
auto main(const int argc, char* argv[]) -> int {
  try {
    ...
    const auto report = v3::run_evaluation(arguments->source_sha, options);
    if (!report) return 70;
    const auto document = v3::serialize_report(*report);
    if (!document || !write_report(arguments->output, *document)) return 70;
    const auto succeeded = v3::evidence_run_succeeded(*report);
